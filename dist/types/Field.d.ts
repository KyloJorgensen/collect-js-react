declare type FieldState = 'dirty' | 'empty' | 'valid' | 'invalid' | 'focused' | 'touched';
export declare type VGSCollectFieldType = 'text' | 'card-number' | 'card-expiration-date' | 'card-security-code' | 'ssn' | 'zip-code' | 'file' | 'password' | 'number' | 'checkbox' | 'radio' | 'dropdown' | 'textarea' | 'date';
export declare type ClassMap = Partial<Record<FieldState, string>>;
declare type VGSCollectKeyboardEventData<T = 'keydown' | 'keypress' | 'keyup'> = {
    type: T;
    timeStamp: number;
    isTrusted: boolean;
    key: string | null;
...
declare type VGSCollectFocusEventData<T = 'focus' | 'blur'> = {
    type: T;
    timeStamp: number;
    isTrusted: boolean;
};
